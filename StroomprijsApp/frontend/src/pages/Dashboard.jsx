...
function ApplianceCalculator({ currentMwh, isMobile }) {
  ...
  const calculate = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/suppliers/calculate", {
        ...
        body: JSON.stringify({ appliances: inputs, region, epex_avg: currentMwh || 100, green_only: greenOnly }),
      });
  ...
  if (result) return <PlanResults result={result} onBack={() => setResult(null)} energyType="electricity" />;

  if (fetchingApp) return <div style={{ textAlign: "center", padding: 40, color: "#556" }}>Loading appliances…</div>;
  ...
  return (
    <div>
      ...
      <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 16, marginBottom: 14 }}>
        <div style={{ color: "#E2E8F0", fontSize: 14, fontWeight: 700, marginBottom: 12 }}>🏠 Your situation</div>
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 6 }}>Region</div>
        <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
          {REGIONS.map(r => (
            <button key={r.id} onClick={() => setRegion(r.id)}
              style={{ flex: 1, padding: "8px 4px", borderRadius: 9, fontSize: 11, fontWeight: 600, cursor: "pointer",
                border: `1px solid ${region === r.id ? "#0D9488" : "rgba(255,255,255,0.08)"}`,
                background: region === r.id ? "rgba(13,148,136,0.15)" : "rgba(255,255,255,0.03)",
                color: region === r.id ? "#0D9488" : "#64748B" }}>
              <div>{r.flag}</div><div>{r.label}</div>
              {!isMobile && <div style={{ fontSize: 9, opacity: 0.7, marginTop: 2 }}>{r.note}</div>}
            </button>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ color: "#E2E8F0", fontSize: 13, fontWeight: 600 }}>🌿 Green energy only</div>
            <div style={{ color: "#64748B", fontSize: 11 }}>Filter to 100% renewable plans</div>
          </div>
          <div onClick={() => setGreenOnly(g => !g)}
            style={{ width: 44, height: 24, borderRadius: 12, background: greenOnly ? "#10B981" : "#1E3A5F", cursor: "pointer", position: "relative", transition: "background 0.2s" }}>
      ...
      {appliances.map(a => {
        ...
        return (
          <div key={a.id} style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${sel.selected ? "rgba(13,148,136,0.3)" : "rgba(255,255,255,0.06)"}`, borderRadius: 12, padding: 12, marginBottom: 8, opacity: sel.selected ? 1 : 0.45, transition: "all 0.15s" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div onClick={() => setSel(a.id, { selected: !sel.selected })}
                style={{ width: 22, height: 22, borderRadius: 6, border: `2px solid ${sel.selected ? "#0D9488" : "rgba(255,255,255,0.2)"}`, background: sel.selected ? "rgba(13,148,136,0.2)" : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {sel.selected && <span style={{ color: "#0D9488", fontSize: 13, lineHeight: 1 }}>✓</span>}
              </div>
              <span style={{ fontSize: 22, flexShrink: 0 }}>{a.icon}</span>
              <div style={{ flex: 1 }}>
                <div style={{ color: "#E2E8F0", fontSize: 13, fontWeight: 600 }}>{a.label}</div>
                <div style={{ color: "#64748B", fontSize: 11 }}>{a.kwh_per_use} kWh/use · peak {a.peak_kw} kW</div>
              </div>
              {sel.selected && (
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                  ...
                  <div style={{ textAlign: "center", minWidth: 36 }}>
                    ...
                    <div style={{ color: "#64748B", fontSize: 9 }}>per week</div>
                  </div>
            ...
            {sel.selected && (
              <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.05)", color: "#64748B", fontSize: 11 }}>💡 {a.tip}</div>
            )}
          </div>
        );
      })}
      ...
      <button onClick={calculate} disabled={loading}
        style={{ width: "100%", padding: "14px 0", borderRadius: 12, border: "none", background: "linear-gradient(135deg,#0D9488,#1A56A4)", color: "#fff", fontSize: 15, fontWeight: 800, cursor: loading ? "default" : "pointer", marginTop: 8, boxShadow: "0 4px 20px rgba(13,148,136,0.35)", opacity: loading ? 0.7 : 1 }}>
...
// ── Manual Compare Tab ────────────────────────────────────────
function ManualCompare({ currentMwh, isMobile }) {
  ...
  useEffect(() => {
    setLoading(true);
    const params = new URLSearchParams({ consumption, region, green: greenOnly, epex: currentMwh || 100 });
    if (typeFilter !== "all") params.set("type", typeFilter);
    fetch(`/api/suppliers/electricity?${params}`)
      .then(r => r.json())
      .then(d => { if (d.success) setResults(d.results); })
      .finally(() => setLoading(false));
  }, [consumption, region, greenOnly, typeFilter, currentMwh]);

  return (
    <div>
      <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 16, marginBottom: 14 }}>
        <div style={{ color: "#E2E8F0", fontSize: 14, fontWeight: 700, marginBottom: 12 }}>⚙️ Settings</div>

        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 6 }}>Annual consumption</div>
        <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
          <input type="number" value={inputVal} onChange={e => setInputVal(e.target.value)}
            ...
            style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#E2E8F0", fontSize: 16, padding: "10px 12px", outline: "none" }} />
          <span style={{ color: "#64748B", fontSize: 13 }}>kWh/yr</span>
        </div>
          ...
          {[2000,3500,5000,8000].map(v => (
            <button key={v} onClick={() => { setConsumption(v); setInputVal(String(v)); }}
              style={{ flex: 1, padding: "6px 0", borderRadius: 7, fontSize: 11, fontWeight: 600, cursor: "pointer",
                border: `1px solid ${consumption===v?"#0D9488":"rgba(255,255,255,0.08)"}`,
                background: consumption===v?"rgba(13,148,136,0.15)":"transparent",
                color: consumption===v?"#0D9488":"#64748B" }}>
              {v>=1000?`${v/1000}k`:v}
            </button>
          ))}
        </div>
        <div style={{ color: "#64748B", fontSize: 11, marginBottom: 12 }}>🏠 Avg: 3,500 kWh/yr · With EV: ~7,000 · With heat pump: ~10,000+</div>
        ...
        <div style={{ color: "#64748B", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 6 }}>Plan type</div>
        <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
          {[["all","All"],["variable","Variable"],["fixed","Fixed"],["dynamic","Dynamic"]].map(([v,l]) => (
            <button key={v} onClick={() => setTypeFilter(v)}
              style={{ flex: 1, padding: "6px 0", borderRadius: 7, fontSize: 11, fontWeight: 600, cursor: "pointer",
                border: `1px solid ${typeFilter===v?"#06B6D4":"rgba(255,255,255,0.08)"}`,
                background: typeFilter===v?"rgba(6,182,212,0.15)":"transparent",
                color: typeFilter===v?"#06B6D4":"#64748B" }}>
              {l}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ color: "#E2E8F0", fontSize: 13, fontWeight: 600 }}>🌿 Green only</span>
          <div onClick={() => setGreenOnly(g => !g)}
            style={{ width: 44, height: 24, borderRadius: 12, background: greenOnly ? "#10B981" : "#1E3A5F", cursor: "pointer", position: "relative" }}>
            <div style={{ position: "absolute", top: 2, left: greenOnly ? 22 : 2, width: 20, height: 20, borderRadius: "50%", background: "#fff", transition: "left 0.2s" }} />
          </div>
        </div>
      </div>

      {meta && <div style={{ color: "#64748B", fontSize: 11, marginBottom: 10, textAlign: "right" }}>Tariff data: {meta.updated} · Next refresh: {meta.next_scrape}</div>}

      {loading ? <div style={{ textAlign: "center", padding: 30, color: "#64748B" }}>Loading plans…</div> : (
        <>
          {results.length > 0 && (
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "16px 8px 8px", marginBottom: 14 }}>
              <div style={{ color: "#64748B", fontSize: 12, paddingLeft: 8, marginBottom: 8 }}>Annual all-in cost · {consumption.toLocaleString()} kWh/yr</div>
              <ResponsiveContainer width="100%" height={Math.min(results.length * 28 + 20, 220)}>
                <BarChart data={results.slice(0,10).map(r=>({ name: r.supplier_name, plan: r.plan_name, total: r.costs.total, color: r.supplier_color }))} layout="vertical" margin={{ left: 0, right: 30, top: 0, bottom: 0 }}>
                  <XAxis type="number" tick={{ fontSize: 10, fill: "#64748B" }} tickFormatter={v=>`€${v}`} domain={["auto","auto"]} />
                  <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fill: "#64748B" }} width={80} />
                  <Tooltip formatter={v=>[`€${v}`,"Annual"]} contentStyle={{ background: "#0D1626", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, fontSize: 11 }} labelStyle={{ color: "#E2E8F0" }} />
                  <Bar dataKey="total" radius={[0,4,4,0]}>
                    {results.slice(0,10).map((r,i) => <Cell key={i} fill={i===0?"#10B981":r.supplier_color} opacity={i===0?1:0.65} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          {results.map((plan, i) => <PlanCard key={plan.plan_id} plan={plan} rank={i} expanded={expanded} setExpanded={setExpanded} />)}
        </>
      )}
      <div style={{ color: "#64748B", fontSize: 11, textAlign: "center", marginTop: 8, lineHeight: 1.6 }}>
        Energy + Fluvius/ORES grid + levies + 6% VAT included.<br />
...
// ── Shared: Plan Results View (after calculator) ──────────────
function PlanResults({ result, onBack, energyType }) {
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "#64748B", borderRadius: 8, padding: "6px 14px", fontSize: 12, cursor: "pointer", marginBottom: 14 }}>
        ← Recalculate
      </button>

      <div style={{ background: "rgba(13,148,136,0.1)", border: "1px solid rgba(13,148,136,0.3)", borderRadius: 16, padding: 20, marginBottom: 16 }}>
        <div style={{ color: "#0D9488", fontSize: 12, fontWeight: 700, marginBottom: 8, textTransform: "uppercase" }}>
          📊 Your estimated {isGas ? "gas" : "electricity"} consumption
        </div>
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          <div>
            <div style={{ color: "#64748B", fontSize: 11 }}>Annual usage</div>
            <div style={{ color: "#E2E8F0", fontSize: 28, fontWeight: 900, fontFamily: "monospace" }}>
              {consumption.total_kwh.toLocaleString()} <span style={{ fontSize: 13, color: "#64748B" }}>kWh</span>
            </div>
          </div>
          {!isGas && consumption.peak_kw && (
            <div>
              <div style={{ color: "#64748B", fontSize: 11 }}>Peak load</div>
              <div style={{ color: "#F59E0B", fontSize: 28, fontWeight: 900, fontFamily: "monospace" }}>
                {consumption.peak_kw} <span style={{ fontSize: 13, color: "#64748B" }}>kW</span>
              </div>
            </div>
          )}
          <div style={{ flex: 1, minWidth: 120 }}>
            <div style={{ color: "#64748B", fontSize: 11 }}>Household type</div>
            <div style={{ color: "#E2E8F0", fontSize: 13, fontWeight: 600, marginTop: 4 }}>{consumption.household_size}</div>
          </div>
        </div>
            ...
            {consumption.breakdown.slice(0,5).map(b => (
              <div key={b.id} style={{ marginBottom: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 2 }}>
                  <span style={{ color: "#E2E8F0" }}>{b.icon} {b.label}</span>
                  <span style={{ color: "#64748B" }}>{b.annual_kwh} kWh ({b.pct}%)</span>
                </div>
                <div style={{ height: 4, background: "rgba(255,255,255,0.06)", borderRadius: 2 }}>
                  <div style={{ height: "100%", width: `${b.pct}%`, background: isGas ? "#F97316" : "#0D9488", borderRadius: 2 }} />
                </div>
              </div>
            ))}
          </div>
      ...
      {cheapest && (
        <div style={{ background: "linear-gradient(135deg,#0A2E20,#0A2040)", border: "1px solid rgba(16,185,129,0.4)", borderRadius: 16, padding: 20, marginBottom: 16 }}>
          <div style={{ color: "#10B981", fontSize: 11, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>🏆 Cheapest for your usage</div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ color: "#E2E8F0", fontSize: 18, fontWeight: 800 }}>{cheapest.supplier_name} — {cheapest.plan_name}</div>
              <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap" }}>
                <PlanBadge color={TYPE_COLOR[cheapest.type]}>{TYPE_LABEL[cheapest.type]}</PlanBadge>
                {cheapest.green && <PlanBadge color="#10B981">🌿 Green</PlanBadge>}
                <PlanBadge color="#64748B">{cheapest.duration}</PlanBadge>
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ color: "#10B981", fontSize: 32, fontWeight: 900 }}>€{cheapest.costs.total}</div>
              <div style={{ color: "#64748B", fontSize: 12 }}>€{cheapest.costs.monthly}/month</div>
            </div>
          </div>
        </div>
      )}

      <div style={{ color: "#E2E8F0", fontSize: 14, fontWeight: 700, marginBottom: 10 }}>All {results.length} plans ranked</div>
      {results.map((plan, i) => <PlanCard key={plan.plan_id} plan={plan} rank={i} expanded={expanded} setExpanded={setExpanded} />)}

      <div style={{ color: "#64748B", fontSize: 11, textAlign: "center", marginTop: 8, lineHeight: 1.7 }}>
        {isGas ? "Includes energy + Fluxys/ORES grid + levies + 21% VAT." : "Includes energy + Fluvius/ORES grid + levies + 6% VAT."}<br />
        Tariff data updated weekly · Verify on supplier website before switching.
      </div>
    </div>
  );
}
...
function PlanCard({ plan, rank, expanded, setExpanded }) {
  ...
  return (
    <div onClick={() => setExpanded(isOpen ? null : plan.plan_id)}
      style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${plan.cheapest ? "#10B981" : "rgba(255,255,255,0.08)"}`, borderRadius: 14, padding: 14, marginBottom: 8, cursor: "pointer", transition: "border-color 0.15s" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
            <span style={{ color: "#E2E8F0", fontSize: 13, fontWeight: 700 }}>{plan.supplier_name}</span>
            <span style={{ color: "#64748B", fontSize: 12 }}>{plan.plan_name}</span>
            {plan.cheapest && <PlanBadge color="#10B981">BEST</PlanBadge>}
            <PlanBadge color={TYPE_COLOR[plan.type]}>{TYPE_LABEL[plan.type]}</PlanBadge>
            {plan.green && <span style={{ color: "#10B981", fontSize: 11 }}>🌿</span>}
          </div>
          <div style={{ color: "#64748B", fontSize: 11, marginTop: 2 }}>
            {plan.energy_rate ? `${(plan.energy_rate*100).toFixed(3)} c€/kWh` : `EPEX/TTF + ${plan.markup_cEkWh}c€`}
            {" · "}€{plan.standing_charge}/yr standing
          </div>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div style={{ color: plan.cheapest ? "#10B981" : "#E2E8F0", fontSize: 17, fontWeight: 800 }}>€{plan.costs.total}</div>
          <div style={{ color: "#64748B", fontSize: 11 }}>€{plan.costs.monthly}/mo</div>
          {rank > 0 && <div style={{ color: "#EF4444", fontSize: 10 }}>+€{plan.savings_vs_cheapest}/yr</div>}
        </div>
      ...
      {isOpen && (
        <div style={{ marginTop: 12, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 10 }}>
            {[["Energy", `€${plan.costs.energy}`, "#F97316"],["Grid", `€${plan.costs.grid}`, "#F59E0B"],["Standing", `€${plan.costs.standing}`, "#64748B"],["VAT", `€${plan.costs.vat}`, "#64748B"]].map(([l,v,c]) => (
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            <div style={{ flex: 1, background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "8px 10px" }}>
              <div style={{ color: "#64748B", fontSize: 10 }}>All-in tariff</div>
              <div style={{ color: "#E2E8F0", fontSize: 14, fontWeight: 700 }}>{plan.costs.perKwh} c€/kWh</div>
            </div>
            <div style={{ flex: 1, background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "8px 10px" }}>
              <div style={{ color: "#64748B", fontSize: 10 }}>Contract</div>
              <div style={{ color: "#E2E8F0", fontSize: 13, fontWeight: 600 }}>{plan.duration}</div>
            </div>
          </div>
          {plan.highlights?.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              {plan.highlights.map(h => <div key={h} style={{ color: "#64748B", fontSize: 12, marginBottom: 3 }}>✓ {h}</div>)}
            </div>
          )}
          {plan.formula && (
            <div style={{ background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.2)", borderRadius: 8, padding: "8px 12px", marginBottom: 10 }}>
              <div style={{ color: "#64748B", fontSize: 10, marginBottom: 2 }}>Pricing formula</div>
              <div style={{ color: "#06B6D4", fontSize: 12, fontFamily: "monospace" }}>{plan.formula}</div>
            </div>
          )}
...
function SupplierCompare({ currentMwh, isMobile }) {
  ...
  return (
    <div>
      ...
      {tab === "calculator" && <ApplianceCalculator currentMwh={currentMwh} isMobile={isMobile} />}
      {tab === "compare"    && <ManualCompare currentMwh={currentMwh} isMobile={isMobile} />}
    </div>
  );
}
