...
export default function AdminDashboard() {
  ...
  const bizPageViews = Number(analytics?.summary?.find(e => e.event === "business_page_view")?.total || 0);
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', system-ui, sans-serif", color: C.text }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.border}`, padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          >
            {[1, 7, 14, 30, 90, 180, 365].map(d => <option key={d} value={d}>{d === 1 ? "Today" : `Last ${d}d`}</option>)}
          </select>
          ...
          <div>
            ...
            {(analytics?.daily_breakdown || []).length > 0 && (() => {
              ...
              for (let i = windowDays - 1; i >= 0; i--) {
                ...
                days[key] = { day: key, ev: 0, page: 0, seo: 0, biz: 0 };
              }
              (analytics.daily_breakdown || []).forEach(row => {
                const d = row.day?.substring(0, 10);
                if (!d) return;
                if (!days[d]) days[d] = { day: d, ev: 0, page: 0, seo: 0, biz: 0 };
                ...
                if (row.event === "business_page_view")  days[d].biz  += Number(row.count);
              });
              const sorted = Object.values(days).sort((a, b) => a.day.localeCompare(b.day)).slice(-14);
              const maxVal = Math.max(...sorted.map(d => Math.max(d.ev, d.page, d.biz)), 1);
              return (
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "20px 24px", marginBottom: 16 }}>
                  <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4 }}>Daily Sessions — Last 14 days</div>
                  <div style={{ fontSize: 11, color: C.muted, marginBottom: 16, display: "flex", gap: 16, flexWrap: "wrap" }}>
                    ...
                    <span><span style={{ display: "inline-block", width: 10, height: 10, borderRadius: 2, background: "#7C3AED", marginRight: 5 }} />Business page</span>
                  </div>
                    ...
                    {sorted.map(d => (
                      <div key={d.day} style={{ flex: "1 0 28px", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, minWidth: 28 }}>
                        <div style={{ width: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", height: 90, gap: 2 }}>
                          ...
                          <div title={`Business: ${d.biz}`} style={{ width: "100%", height: `${Math.round((d.biz / maxVal) * 90)}px`, background: "#7C3AED", borderRadius: "3px 3px 0 0", minHeight: d.biz > 0 ? 2 : 0 }} />
                        </div>
