...
function getPriceLabel(mwh) {
  if (mwh < 0)   return { emoji: "ðŸ¤‘", text: "Negative price" };
  if (mwh < 50)  return { emoji: "ðŸ’š", text: "Very cheap" };
  if (mwh < 90)  return { emoji: "ðŸŸ¡", text: "Cheap" };
  if (mwh < 130) return { emoji: "ðŸŸ ", text: "Moderate" };
  if (mwh < 160) return { emoji: "ðŸ”´", text: "Expensive" };
  return           { emoji: "â›”", text: "Peak price" };
}

const CONTENT = {
  en: {
    title: "EPEX Spot Belgium â€” Live Electricity Price",
    ...
    faq3: "The price shown is the wholesale market price in â‚¬/MWh (â‚¬ per megawatt-hour). Your retail tariff from a supplier like Engie, Luminus or Bolt Energy will be higher due to grid costs, taxes and the supplier markup.",
    cta: "Compare all 7 Belgian supplier plans â†’",
  ...
  nl: {
    title: "EPEX Spot BelgiÃ« â€” Live elektriciteitsprijs",
    desc: "Realtime EPEX Spot day-ahead elektriciteitsprijs voor BelgiÃ«. Elk uur bijgewerkt. Gratis.",
    ...
    faqTitle: "Wat is EPEX Spot BelgiÃ«?",
    ...
    faq2: "Als u een dynamisch energiecontract heeft in BelgiÃ«, verandert uw kosten per kWh elk uur op basis van deze EPEX-prijzen. SmartPrice.be volgt deze prijzen in realtime zodat u weet wanneer u het goedkoopst uw toestellen kunt gebruiken.",
    faq3: "De getoonde prijs is de groothandelsprijs in â‚¬/MWh. Uw retailtarief bij een leverancier zoals Engie, Luminus of Bolt Energy is hoger door netkosten, belastingen en de leveranciersopslag.",
    cta: "Vergelijk alle 7 Belgische tarieven â†’",
    ctaSub: "Gratis tariefvergelijking voor uw verbruiksprofiel",
  },
  fr: {
    title: "EPEX Spot Belgique â€” Prix de l'Ã©lectricitÃ© en direct",
    desc: "Prix EPEX Spot day-ahead en temps rÃ©el pour la Belgique. Mis Ã  jour chaque heure. Gratuit.",
    currentLabel: "Prix EPEX Spot actuel",
    updated: "Mis Ã  jour",
    todayLabel: "Plage d'aujourd'hui",
    min: "Min", avg: "Moy", max: "Max",
    cheapestTitle: "5 heures les moins chÃ¨res aujourd'hui",
    cheapestDesc: "Meilleures plages pour recharger un VE, lave-linge, lave-vaisselle",
    faqTitle: "Qu'est-ce qu'EPEX Spot Belgique ?",
    faq1: "EPEX Spot est la bourse europÃ©enne de l'Ã©nergie oÃ¹ l'Ã©lectricitÃ© est Ã©changÃ©e sur le marchÃ© de gros day-ahead. Les prix de l'Ã©lectricitÃ© belge (Ã©galement appelÃ©s BELPEX) sont fixÃ©s chaque jour pour les 24 heures suivantes.",
    faq2: "Si vous avez un contrat d'Ã©nergie dynamique en Belgique, votre coÃ»t par kWh change chaque heure en suivant ces prix EPEX. SmartPrice.be suit ces prix en temps rÃ©el pour que vous sachiez quand faire tourner vos appareils.",
    faq3: "Le prix affichÃ© est le prix de gros en â‚¬/MWh. Votre tarif de dÃ©tail chez un fournisseur comme Engie, Luminus ou Bolt Energy sera plus Ã©levÃ© en raison des frais de rÃ©seau, des taxes et de la marge du fournisseur.",
    cta: "Comparer les 7 fournisseurs belges â†’",
...
export default function EpexBelgiumPage({ onGetStarted, onOpenCalculator }) {
  ...
  useEffect(() => {
    document.title = "EPEX Spot Belgium â€” Live Electricity Price Today | SmartPrice.be";
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <nav style={{ borderBottom: `1px solid ${C.border}`, padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(6,11,20,0.9)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <LangSwitcher />
          <a href="/" onClick={e => { e.preventDefault(); onGetStarted(); }} style={{ padding: "8px 18px", borderRadius: 20, fontSize: 13, fontWeight: 700, background: `linear-gradient(135deg,${C.teal},#1A56A4)`, color: "#fff", textDecoration: "none", cursor: "pointer" }}>Dashboard â†’</a>
        </div>
      </nav>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "40px 20px" }}>
        ...
        <div style={{ background: `linear-gradient(135deg, ${col}14, ${col}06)`, border: `1px solid ${col}44`, borderRadius: 24, padding: "32px 36px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 20 }}>
          <div>
            ...
            {loading ? (
              <div style={{ fontSize: 56, fontWeight: 900, color: C.muted }}>â€”</div>
            ) : mwh != null ? (
              <>
                <div style={{ fontSize: "clamp(48px,8vw,72px)", fontWeight: 900, fontFamily: "monospace", color: col, lineHeight: 1 }}>
                  â‚¬{mwh.toFixed(1)}
                </div>
                <div style={{ fontSize: 16, color: col, marginTop: 6 }}>{lbl?.emoji} {lbl?.text} Â· /MWh</div>
              </>
            ) : (
              <div style={{ fontSize: 32, color: C.muted }}>â€”</div>
            )}
            {current && (
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              {[
                ...
                <div key={s.label} style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 24, fontWeight: 800, color: s.color, fontFamily: "monospace" }}>â‚¬{s.value}</div>
        ...
        {prices.length > 0 && (
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20, padding: "20px 24px", marginBottom: 24 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 16, color: C.muted }}>{T.todayLabel} Â· Belgium EPEX Spot</div>
              ...
              {prices.filter((_, i) => i % 1 === 0).map((p, i) => {
                ...
                return (
                  <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                    <div title={`${p.hour_label} â€” â‚¬${p.price_eur_mwh?.toFixed(1)}/MWh`} style={{ width: "100%", height: h, background: p.is_current ? "#fff" : getPriceColor(p.price_eur_mwh), borderRadius: "3px 3px 0 0", opacity: p.is_current ? 1 : 0.7 }} />
                  </div>
                );
              })}
            </div>
        ...
        {cheapest.length > 0 && (
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20, padding: "20px 24px", marginBottom: 24 }}>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>ðŸ’š {T.cheapestTitle}</div>
              ...
              {cheapest.slice(0, 5).map((h, i) => {
                ...
                return (
                  <div key={i} style={{ flex: 1, minWidth: 100, background: `${col2}10`, border: `1px solid ${col2}30`, borderRadius: 14, padding: "12px 14px", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 900, color: col2, fontFamily: "monospace", marginTop: 4 }}>â‚¬{h.price_eur_mwh.toFixed(1)}</div>
                  </div>
                );
              })}
            </div>
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "@type": "WebPage",
          "name": "EPEX Spot Belgium â€” Live Electricity Price",
