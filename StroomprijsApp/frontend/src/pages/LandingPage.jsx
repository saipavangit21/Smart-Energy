/**
 * LandingPage.jsx â€” SmartPrice.be
 * v3: decision-first hero. The ANSWER is the headline.
 * Philosophy: show "charge between 14:00â€“16:00" before anything else.
 */
...
export default function LandingPage({ onGetStarted, onOpenCalculator }) {
  ...
  useEffect(() => {
    document.title = "SmartPrice.be â€” Live Belgian Electricity & Gas Prices | Free";
  ...
  // Countdown ticker â€” updates every 60s
  useEffect(() => {
  ...
  // Use backend-computed `hour` (Brussels timezone) â€” never rely on browser getHours() for price slots
  ...
  if (prices.length) {
    if (cheapNow) {
      urgencyText = L.urgencyNow || "âš¡ CHEAPEST WINDOW IS RIGHT NOW â€” Plug in now";
      urgencyColor = "#10B981";
    } else if (cheapSoon) {
      urgencyText = (L.urgencySoon || "âš¡ Cheapest window starts in {x} â€” get ready").replace("{x}", countdownStr);
      urgencyColor = "#F97316";
    } else if (cheapHour != null) {
      urgencyText = (L.urgencyLater || "Today's cheapest: {time} Â· â‚¬{price}/MWh").replace("{time}", fmtHour(cheapHour)).replace("{price}", cheapMwh?.toFixed(0));
      urgencyColor = "#0D9488";
    }
  }

  // "Updated X min ago" â€” recalculates every tick (every 60s)
  ...
  const SUPPLIERS = [
    ...
    { name: "Bolt",          abbr: "âš¡", color: "#fff",    bg: "#1A1A2E", accent: "#00C896" },
  ...
  const CTAButton = ({ label = "Start saving on every charge â†’", style = {} }) => (
    ...
    <div style={{ minHeight: "100vh", background: "#060B14", color: "#E8EDF5", fontFamily: "'DM Sans', system-ui, sans-serif", overflowX: "hidden" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(6,11,20,0.94)", backdropFilter: "blur(24px)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "11px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 9, color: "#10B981", background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.3)", borderRadius: 20, padding: "2px 8px", fontWeight: 700 }}>â— LIVE</span>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <ThemeSwitcher />
          <LangSwitcher style={{ marginRight: 4 }} />
          <a href="/ev-charging-belgium" style={{ padding: "7px 13px", borderRadius: 20, fontSize: 12, fontWeight: 700, background: "rgba(0,200,150,0.08)", border: "1px solid rgba(0,200,150,0.22)", color: "#00C896", textDecoration: "none" }}>ðŸš— EV</a>
      ...
      {/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
          HERO â€” THE DECISION ENGINE
          First thing visible = "Charge between 14:00â€“16:00"
      â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
        ...
        <div style={{ opacity: heroIn ? 1 : 0, transform: heroIn ? "translateY(0)" : "translateY(16px)", transition: "all 0.6s ease" }}>
          ...
          {currentMwh != null && currentMwh < 0 && (
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 14 }}>
              <div style={{
                ...
                <span>
                  Electricity is <strong>FREE</strong> right now â€” {currentMwh.toFixed(1)} â‚¬/MWh
                  &nbsp;Â·&nbsp;
                  <span style={{ textDecoration: "underline" }}>
                    {lang === "nl" ? "Laad nu op â†’" : lang === "fr" ? "Rechargez maintenant â†’" : "Charge now â†’"}
                  </span>
                </span>
              </div>
            </div>
          ...
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.3)", borderRadius: 20, padding: "5px 14px", fontSize: 12, fontWeight: 700, color: "#F59E0B" }}>
              <span>âš¡</span><span>Live Belgian electricity prices â€” updated every 15 min</span>
            </div>
          </div>
          ...
          <p style={{ fontSize: "clamp(14px,2vw,18px)", color: "#556B82", textAlign: "center", margin: "0 0 20px", fontWeight: 500 }}>
            {L.tagline || "We tell you exactly when to charge â€” and how much you save."}
          </p>

          {/* Product scope row */}
          <div style={{ display: "flex", justifyContent: "center", gap: 8, flexWrap: "wrap", marginBottom: 28 }}>
            {[
              { icon: "âš¡", label: L.pillElec || "Electricity prices", color: "#F59E0B", bg: "rgba(245,158,11,0.1)", border: "rgba(245,158,11,0.25)" },
              { icon: "ðŸ”¥", label: L.pillGas  || "Gas prices",         color: "#F97316", bg: "rgba(249,115,22,0.1)", border: "rgba(249,115,22,0.25)" },
              { icon: "ðŸš—", label: L.pillEv   || "EV charging",        color: "#10B981", bg: "rgba(16,185,129,0.1)", border: "rgba(16,185,129,0.25)" },
            ].map(p => (
              ...
              <div style={{ padding: "24px 24px 20px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 700, color: "#F59E0B" }}>
                    <span>âš¡</span><span>Electricity Â· EPEX Spot Belgium</span>
                  </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
                  ...
                  <div style={{ background: "rgba(39,39,42,0.9)", border: "1px solid rgba(63,63,70,0.6)", borderRadius: 14, padding: "16px 18px" }}>
                    <div style={{ fontSize: 11, color: "#71717a", marginBottom: 6, fontWeight: 600 }}>
                      {cheapIsNow ? "âš¡ Cheapest â€” plug in now" : (cheapEntry?.day === "tomorrow" ? "Best window (tomorrow)" : "Best window today")}
                    </div>
                    <div style={{ fontSize: 20, fontWeight: 700, color: "#f4f4f5", marginBottom: 4, fontFamily: "monospace", letterSpacing: "-0.5px" }}>
                      {cheapIsNow ? "Right now!" : `${fmtHour(cheapHour)} â€“ ${fmtHour((cheapWindowEnd ?? cheapHour + 2))}`}
                    </div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#4ade80", animation: "priceGlow 3s ease-in-out infinite" }}>â‚¬{retailFmt(cheapMwh)}/kWh</div>
                  </div>
                  ...
                  <div style={{ background: "rgba(39,39,42,0.9)", border: "1px solid rgba(63,63,70,0.6)", borderRadius: 14, padding: "16px 18px" }}>
                    ...
                    <div style={{ fontSize: 20, fontWeight: 700, color: "#f4f4f5", marginBottom: 4, fontFamily: "monospace", letterSpacing: "-0.5px" }}>
                      {peakH != null ? `${fmtHour(peakH)} â€“ ${fmtHour(peakH + 2)}` : "â€”"}
                    </div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#f87171" }}>â‚¬{peakMwh != null ? retailFmt(peakMwh) : "â€”"}/kWh</div>
                  </div>
                </div>
                ...
                <div style={{ background: "rgba(39,39,42,0.9)", border: "1px solid rgba(63,63,70,0.6)", borderRadius: 14, padding: "16px 18px", marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 16, fontWeight: 700, color: "#4ade80" }}>â‚¬{(retailKwh(cheapMwh) * 40).toFixed(2)}</span>
                  </div>
                  {currentMwh != null && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      <span style={{ fontSize: 14, color: "#a1a1aa" }}>{L.rightNowLabel || "Right now"} ({fmtHour(nowH)})</span>
                      <span style={{ fontSize: 16, fontWeight: 700, color: currentCol }}>â‚¬{(retailKwh(currentMwh) * 40).toFixed(2)}</span>
                    </div>
                  )}
                  {savingToday != null && savingToday > 0.3 && (
                    <div style={{ borderTop: "1px solid rgba(63,63,70,0.5)", paddingTop: 10, fontSize: 15, fontWeight: 700, color: "#4ade80" }}>
                      {(L.youSave || "â†’ You save â‚¬{x} per charge").replace("{x}", savingToday.toFixed(2))}
                    </div>
                  )}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, padding: "10px 14px", background: hasSolar ? "rgba(251,191,36,0.08)" : "rgba(255,255,255,0.03)", border: `1px solid ${hasSolar ? "rgba(251,191,36,0.3)" : "rgba(63,63,70,0.5)"}`, borderRadius: 12, cursor: "pointer", transition: "all 0.2s" }}
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: hasSolar ? "#fbbf24" : "#71717a" }}>
                    â˜€ï¸ {L.hasSolarLabel || "I have solar panels"}
                  </span>
                ...
                {hasSolar && cheapMwh != null && (
                  <div style={{ background: "rgba(251,191,36,0.07)", border: "1px solid rgba(251,191,36,0.25)", borderRadius: 12, padding: "12px 14px", marginBottom: 14 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#fbbf24", marginBottom: 4 }}>{L.capacityTariffLabel || "â˜€ï¸ Capacity tariff (Fluvius) impact"}</div>
                    <div style={{ fontSize: 12, color: "#a16207", lineHeight: 1.7 }}>
                      {(L.capacityTariffDesc || "Charging at peak ({peak}) sets your monthly peak demand â€” adding ~â‚¬20â€“30 to your Fluvius bill. Charging at {cheap} avoids this.").replace("{peak}", fmtHour(peakH ?? 19)).replace("{cheap}", fmtHour(cheapHour))} <span style={{ color: "#fbbf24", fontWeight: 700 }}>{(L.capacityTariffRealSaving || "Real saving: â‚¬{x}/month.").replace("{x}", ((savingToday ?? 0) + 22).toFixed(0))}</span>
                    </div>
                    <div style={{ fontSize: 11, color: "#713f12", marginTop: 6 }}>{L.fluviusComingSoon || "Full Fluvius integration coming soon â†’"}</div>
                  </div>
                )}
                ...
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#60a5fa", marginBottom: 14 }}>
                    {(L.chargeBeforeCta || "ðŸ‘‰ Charge before {x} to save money").replace("{x}", fmtHour(cheapWindowEnd ?? cheapHour + 2))}
                  </div>
                  <CTAButton label={L.mainCta || "Start saving on every charge â†’"} style={{ width: "100%", fontSize: 15, padding: "14px 0", borderRadius: 14 }} />
                  <div style={{ fontSize: 11, color: "#3f3f46", marginTop: 10 }}>{L.freeNote || "Free Â· No account needed Â· 30 sec to set alerts"}</div>
                </div>

              </div>
            ) : (
              <div style={{ padding: "52px 24px", textAlign: "center" }}>
                <div style={{ fontSize: 13, color: "#52525b" }}>Loading live pricesâ€¦</div>
              </div>
            )}

            {/* Card footer â€” trust signal + live timestamp */}
          ...
          {/* Habit trigger â€” tomorrow's prices */}
          {showTomorrowTeaser && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "10px 16px", background: "rgba(59,130,246,0.07)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 12, marginTop: 10, fontSize: 12, color: "#60a5fa", fontWeight: 600 }}>
              <span>ðŸ“…</span>
              <span>Tomorrow's cheapest hours publish at {String(tomorrowPublishHour).padStart(2,"0")}:00 â€” check back then</span>
            </div>
          )}

          {/* Trust */}
          <div style={{ display: "flex", gap: 18, justifyContent: "center", flexWrap: "wrap", marginBottom: 8 }}>
            {["ðŸ‡§ðŸ‡ª Belgian data", "ðŸ†“ Free forever", "âš¡ Every 15 min", "ðŸ”’ GDPR"].map(b => (
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div>
                <div style={{ fontSize: 11, color: "#71717a", fontWeight: 700, marginBottom: 2 }}>TTF Natural Gas Â· Today</div>
                <div style={{ fontSize: 20, fontWeight: 800, color: "#f97316", fontFamily: "monospace", letterSpacing: "-0.5px" }}>
                  {gasCurrent ? `â‚¬${gasCurrent.price?.toFixed(1)}/MWh` : "â€”"}
                </div>
                {gasCurrent?.ttf_cEkWh != null && (
                  <div style={{ fontSize: 12, color: "#78716c", marginTop: 1 }}>= {gasCurrent.ttf_cEkWh.toFixed(3)} câ‚¬/kWh energy</div>
                )}
              </div>
            </div>
          </div>
          {/* Right â€” bill breakdown hint + CTA */}
          ...
          {leadState === "done" ? (
            <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "#445566", marginTop: 3 }}>{L.onListDesc || "We'll alert you when tomorrow's cheapest window is published â€” every day at 13:00 CET."}</div>
              </div>
            </div>
          ) : (
            <form onSubmit={submitLead} style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 220 }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#E2E8F0", marginBottom: 6 }}>
                  {L.alertCaptureTitle || "ðŸ”” Alert me when the cheapest window opens"}
                </div>
                <div style={{ fontSize: 12, color: "#445566" }}>{L.alertCaptureSub || "Daily at 13:00 CET Â· No spam Â· Unsubscribe anytime"}</div>
              </div>
              <div style={{ display: "flex", gap: 8, flex: 1, minWidth: 260 }}>
                ...
                <button type="submit" disabled={leadState === "loading"} style={{
                  ...
                  {leadState === "loading" ? "â€¦" : (L.notifyBtn || "Notify me â†’")}
                </button>
              </div>
              {leadState === "error" && (
                <div style={{ width: "100%", fontSize: 12, color: "#EF4444" }}>{L.alertError || "Something went wrong â€” try again or email hello@smartprice.be"}</div>
              )}
            </form>
          )}
        </div>
      ...
      <section ref={planRef} style={{ maxWidth: 860, margin: "28px auto 0", padding: "0 20px", position: "relative", zIndex: 1 }}>
        <Tile accent="#00C896" icon="ðŸ”‹" label={L.evPlannerTitle || "EV Charge Planner â€” tell us your battery, we find the window"}>
          ...
          {planResult ? (
            <div style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.3)", borderRadius: 16, padding: "22px 24px" }}>
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 10, color: "#10B981", textTransform: "uppercase", letterSpacing: "1.5px", fontWeight: 700, marginBottom: 6 }}>{L.optimalWindow || "âš¡ Charge between"}</div>
                  <div style={{ fontSize: "clamp(28px,5vw,44px)", fontWeight: 900, color: "#10B981", fontFamily: "monospace", letterSpacing: "-1.5px", lineHeight: 1, marginBottom: 4 }}>
                    {fmtHour(planResult.start)} â€“ {fmtHour(planResult.end)}
                  </div>
                  <div style={{ fontSize: 12, color: "#334455" }}>{(L.hoursKwh || "{h}h Â· {kwh} kWh needed").replace("{h}", planResult.hours).replace("{kwh}", planResult.needed.toFixed(0))}</div>
                </div>
                <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 10, color: "#445566", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 4 }}>{L.costLabel || "Cost"}</div>
                    <div style={{ fontSize: 28, fontWeight: 900, color: "#10B981", fontFamily: "monospace" }}>â‚¬{planResult.cost.toFixed(2)}</div>
                  </div>
                  {planResult.saving > 0.1 && (
                    <div style={{ textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 28, fontWeight: 900, color: "#F97316", fontFamily: "monospace" }}>-â‚¬{planResult.saving.toFixed(2)}</div>
                    </div>
                  )}
                  {planResult.saving > 0.1 && (
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 10, color: "#445566", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 4 }}>{L.perYearLabel || "Per year Ã—250"}</div>
                      <div style={{ fontSize: 28, fontWeight: 900, color: "#F97316", fontFamily: "monospace" }}>-â‚¬{(planResult.saving * 250).toFixed(0)}</div>
                    </div>
                  )}
                </div>
              </div>
              <CTAButton label={L.alertsBtn || "Get alerts for this window â†’"} />
              <div style={{ fontSize: 11, color: "#2A3A4A", marginTop: 8, textAlign: "center" }}>{L.freeNoCard || "Free Â· No credit card"}</div>
            </div>
          ) : (
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "18px", textAlign: "center" }}>
              <span style={{ fontSize: 13, color: "#445566" }}>{prices.length ? (L.adjustInputs || "Adjust inputs to calculate your window") : (L.loadingPrices || "Loading pricesâ€¦")}</span>
            </div>
          )}
        </Tile>
      </section>
      ...
      {prices.length > 0 && (
        <section style={{ maxWidth: 860, margin: "28px auto 0", padding: "0 20px", position: "relative", zIndex: 1 }}>
          <Tile accent="#1A56A4" icon="ðŸ“Š" label={L.scheduleTitle || "Today's full schedule â€” 24 hours"}>
              ...
              {prices.slice(0, 24).map((p, i) => {
                ...
                return (
                  <div key={i} style={{ flex: 1, height: `${barH}%`, borderRadius: "3px 3px 0 0", background: p.is_current ? col : isCheap ? col : `${col}55`, border: (p.is_current || isCheap) ? `1px solid ${col}` : "none", boxShadow: (p.is_current || isCheap) ? `0 0 8px ${col}88` : "none" }}
                    title={`${String(pHour).padStart(2,"0")}:00 â€” â‚¬${p.price_eur_mwh?.toFixed(0)}/MWh`} />
                );
              })}
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
              <div style={{ display: "flex", gap: 14, fontSize: 11, color: "#3A4D63" }}>
                <span><span style={{ color: "#10B981" }}>â– </span> {L.legendCheap || "Cheap"}</span>
                <span><span style={{ color: "#F59E0B" }}>â– </span> {L.legendMid || "Mid"}</span>
                <span><span style={{ color: "#EF4444" }}>â– </span> {L.legendPeak || "Peak"}</span>
                <span><span style={{ color: "#fff" }}>â– </span> {L.legendNow || "Now/Best"}</span>
              </div>
              <button onClick={onGetStarted} style={{ padding: "7px 18px", borderRadius: 20, fontSize: 12, fontWeight: 700, background: "rgba(0,200,150,0.1)", border: "1px solid rgba(0,200,150,0.3)", color: "#00C896", cursor: "pointer" }}>
                {L.openDashBtn || "Open full dashboard â†’"}
              </button>
            </div>
          </Tile>
        </section>
      ...
      â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      <section style={{ maxWidth: 860, margin: "28px auto 0", padding: "0 20px", position: "relative", zIndex: 1 }}>
        <Tile accent="#0D9488" icon="ðŸ”Œ" label="Plan Calculator" onClick={() => onOpenCalculator && onOpenCalculator("electricity")}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
            <div>
              ...
              <div style={{ fontSize: 13, color: "#556B82" }}>{L.calcDesc2 || "Compare all 7 Belgian suppliers Â· real annual cost Â· 30 sec"}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 22, fontWeight: 900, color: "#10B981", fontFamily: "monospace" }}>â‚¬987 <span style={{ fontSize: 12, color: "#334455", fontFamily: "inherit", fontWeight: 400 }}>vs â‚¬1,204</span></div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#0D9488", marginTop: 2 }}>{L.calcCta || "Calculate my plan â†’"}</div>
            </div>
          </div>
        </Tile>
      </section>
      ...
      <section style={{ maxWidth: 860, margin: "40px auto 0", padding: "0 20px", position: "relative", zIndex: 1 }}>
        <div style={{ background: "linear-gradient(135deg, rgba(16,185,129,0.1), rgba(13,148,136,0.06))", border: "1px solid rgba(16,185,129,0.25)", borderRadius: 20, padding: "36px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 11, color: "#2A3A4A", marginTop: 12 }}>{L.freeUnsubscribe || "Free Â· No credit card Â· Unsubscribe anytime"}</div>
        </div>
      </section>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 240 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.35)", borderRadius: 20, padding: "4px 12px", fontSize: 11, fontWeight: 800, color: "#fbbf24", marginBottom: 12, letterSpacing: "1px", textTransform: "uppercase" }}>
                {L.fluviusComingSoonBadge || "âš¡ Coming soon"}
              </div>
              ...
              <div style={{ fontSize: 13, color: "#71717a", lineHeight: 1.75, marginBottom: 16 }}>
                {L.fluviusDesc || "Since 2023, Belgian households with a smart meter pay a capacity tariff â€” your monthly grid bill is based on your highest 15-min peak. Charging your EV at the wrong hour doesn't just cost more per kWh â€” it raises your entire month's Fluvius bill by â‚¬20â€“30. We're building full Fluvius integration to show you the real cost of every charging decision."}
              </div>
              <form onSubmit={submitFluvius} style={{ display: "flex", gap: 8 }}>
                {fluviusState === "done" ? (
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#fbbf24" }}>{L.fluviusWaitlistDone || "âœ… You're on the Fluvius waitlist"}</div>
                ) : (
                  <>
                    <input
                    ...
                    <button type="submit" disabled={fluviusState === "loading"} style={{ padding: "10px 18px", borderRadius: 10, fontSize: 13, fontWeight: 700, background: "linear-gradient(135deg,#f59e0b,#d97706)", border: "none", color: "#1c1917", cursor: "pointer", whiteSpace: "nowrap", opacity: fluviusState === "loading" ? 0.7 : 1 }}>
                      {fluviusState === "loading" ? "â€¦" : (L.notifyBtn || "Notify me â†’")}
                    </button>
                  </>
                )}
              </form>
            ...
            <div style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)", borderRadius: 14, padding: "16px 18px", minWidth: 180 }}>
              ...
              {(L.fluviusFeatures || [
                ...
                <div key={f} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#78716c", marginBottom: 7 }}>
                  <span style={{ color: "#f59e0b" }}>â†’</span> {f}
                </div>
              ))}
            </div>
          </div>
      ...
      <footer style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "44px 24px 28px", marginTop: 52, position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 28, marginBottom: 28 }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <span style={{ fontSize: 17 }}>ðŸ‡§ðŸ‡ª</span>
            ...
            <div style={{ display: "flex", gap: 40, flexWrap: "wrap" }}>
              <div>
                ...
                {[
                  { label: "âš¡ Electricity Prices", action: onGetStarted },
                  { label: "ðŸ”¥ Gas Prices",         action: onGetStarted },
                  { label: "ðŸ”Œ Plan Calculator",    action: () => onOpenCalculator && onOpenCalculator("electricity") },
                  { label: "ðŸš— EV Charging",        action: () => window.location.href = "/ev-charging-belgium" },
                  { label: "ðŸ—ºï¸ Charging Stations",  action: () => window.location.href = "/ev-charging-stations-belgium" },
                ].map(l => (
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.04)", paddingTop: 18, marginBottom: 16, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            {[
              { label: "WhatsApp", icon: "ðŸ’¬", color: "#25D366", href: `https://wa.me/?text=${encodeURIComponent("âš¡ SmartPrice.be â€” tells you exactly when to charge your EV to save money. Live EPEX prices, free. https://smartprice.be")}` },
              { label: "X",        icon: "ð•",  color: "#fff",    href: `https://twitter.com/intent/tweet?text=${encodeURIComponent("âš¡ SmartPrice.be â€” live electricity prices for Belgium + EV charge planner. Free. https://smartprice.be")}` },
              ...
              { label: "Email",    icon: "âœ‰",  color: "#94A3B8", href: `mailto:?subject=${encodeURIComponent("SmartPrice.be â€” free EV charge optimizer for Belgium")}&body=${encodeURIComponent("Check this out: SmartPrice.be tells you exactly when to charge your EV to save the most. Free tool for Belgium â€” https://smartprice.be")}` },
            ].map(s => (
          ...
          <div style={{ fontSize: 11, color: "#2A3A4A", lineHeight: 2 }}>
            {L.footer || "Data: Energy-Charts.info Â· Elia Open Data (CC BY 4.0) Â· ENTSO-E Â· Prices refresh every 15 min"}<br />
